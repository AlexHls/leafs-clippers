...
def make_plot(ind, snappath="output", model="one_def", ye_min=0.25, plotdir="plots"):
    ...
    for i in range(2):
        for j in range(4):
            inner_grid = outer_grid[i, j].subgridspec(1, 2, width_ratios=[1, 0.05], wspace=0, hspace=0)
            ax = fig.add_subplot(inner_grid[0, 0])
            cax = fig.add_subplot(inner_grid[0, 1])
            axes.append(ax)
            caxes.append(cax)

    s.plot_slice("density", ax=axes[0], cax=caxes[0], boxsize=bs, log=True, show_time=False)
    s.plot_slice("energy", ax=axes[1], cax=caxes[1], boxsize=bs, log=True, show_time=False)
    s.plot_slice("ye", ax=axes[2], cax=caxes[2], boxsize=bs, log=False, show_time=False, Min=ye_min, Max=0.5)
    s.plot_slice("Amean", ax=axes[3], cax=caxes[3], boxsize=bs, log=False, show_time=False, Min=1, Max=56)
    ...
    s.plot_slice("ye", ax=axes[6], log=False, show_time=False, cax=caxes[6], Min=ye_min, Max=0.5)
