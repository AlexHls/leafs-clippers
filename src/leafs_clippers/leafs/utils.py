...
class LeafsXdmf3Writer:
    ...
    def _write_header(self, f):
        ...
        f.write('<Grid Name="3DStructuredGrid" GridType="Collection" CollectionType="Spatial">\n')
    ...
    def _write_geometry(self, f, grid_shape: tuple, offset: tuple, filename: str) -> None:
        ...
        for dim, edge, start in zip(grid_shape, ["edgex", "edgey", "edgez"], offset):
            ...
            f.write("</DataItem>\n")
            f.write('<DataItem Dimensions="{}" NumberType="Float" Precision="8" Format="HDF">\n'.format(dim))
            f.write(f"{filename}:/{edge}\n")
            f.write("</DataItem>\n")
            f.write("</DataItem>\n")
        f.write("</Geometry>\n")

    def _write_attribute(self, f, label: str, grid_shape: tuple, offset: tuple, filename: str) -> None:
        f.write(f'<Attribute Name="{label}" AttributeType="Scalar" Center="Cell">\n')
        f.write('<DataItem ItemType="HyperSlab" Dimensions="{} {} {}">\n'.format(grid_shape[0]-1, grid_shape[1]-1, grid_shape[2]-1))
        f.write('<DataItem Dimensions="3 3" NumberType="Int" Format="XML">\n')
        f.write(
            f"{offset[0]} {offset[1]} {offset[2]} 1 1 1 {grid_shape[0]-1} {grid_shape[1]-1} {grid_shape[2]-1}\n"
        )  # Start, Stride, Count
        f.write("</DataItem>\n")
        f.write(
            ...
            for offset, subgrid_shape in self._split_grid():
                ...
                for label in self.snapshot.keys:
                    if label in self._ignore_keys:
                        continue
                    self._write_attribute(f, label, subgrid_shape, offset, self.filename)
                f.write("</Grid>\n")
